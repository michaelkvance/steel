...
-- let's add a thruster and fix the acceleration curve
...
local function ship_thrust(ship, impulse)
	local IMPULSE_MAX = 0.25
	if impulse ~= 0 then
		ship.thrust = ship.thrust + clamp(impulse, -IMPULSE_MAX, IMPULSE_MAX)
	else
		ship.thrust = 0
	end
	local normal = ship.angles.normal
	local deltav = vec2d_scale(normal, ship.thrust)
	local velocity = vec2d_add(ship.velocity, deltav)
	local magnitude = vec2d_length(velocity)
	local VELOCITY_MAX = 4.0
	if magnitude > VELOCITY_MAX then
		velocity = vec2d_scale(velocity, VELOCITY_MAX/magnitude)
	end
...
local function ship_draw(ship)
	...
	if ship.thrust > 0 then
		local exhaust = vec2d_add(origin, vec2d_scale(normal, -3.0))
		local flare = vec2d_add(exhaust, vec2d_scale(normal, -2.0))
		line(exhaust.x, exhaust.y, flare.x, flare.y, 8)
	end
